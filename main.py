...
class CDPClient:
                ...
                try:
                    ...
                    if line:
                        ...
                        if len(self.buffer) >= BUFFER_DURATION_S: # process 15 min worth of data
                            plugin.publish("rawzb64.data", rawzb64_data, timestamp=timestamp)
                            with open("/tmp/data_15min_rawzb64_data.txt", "w") as file:
                                for time_collect,data_collect in self.buffer:
                                    file.write(f"{time_collect},{data_collect}\n")
                            plugin.upload_file("/tmp/data_15min_rawzb64_data.txt", timestamp=timestamp)
    ...
    def process_buffer(self,plugin):
        if not self.buffer:
        ...
        timestamp = time.time_ns()
        plugin.publish("cdp.lwc", lwc, timestamp=timestamp)
        plugin.publish("cdp.fog_present", str(fog_present), timestamp=timestamp)
